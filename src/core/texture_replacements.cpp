...
static std::string GetDefaultConfigurationString();
...
void TextureReplacements::DumpTexture(ReplacementType type, u32 offset_x, u32 offset_y, u32 src_width, u32 src_height,
                                      GPUTextureMode mode, TextureSourceHash src_hash, TexturePaletteHash pal_hash,
                                                          ...
                                                          const GSVector2i& offset_to_page)
{
  ...
  for (auto it = begin; it != end; ++it)
  {
    ...
    replacements.push_back(
      ReplacementSubImage{rect_in_page_space, GSVector4i::zero(), *image, scale.x, scale.y, name.IsSemitransparent()});
  }
}
...
void TextureReplacements::GetTexturePageTextureReplacements(std::vector<ReplacementSubImage>& replacements,
                                                            u32 start_page_number, TextureSourceHash page_hash,
                                                            TextureSourceHash palette_hash, GPUTextureMode mode,
                                                            GPUTexturePaletteReg palette)
{
  ...
  const GSVector4i page_start_in_vram =
    GSVector4i(VRAMPageStartX(start_page_number), VRAMPageStartY(start_page_number)).xyxy();
...
bool TextureReplacements::EnsureGameDirectoryExists()
{
  ...
  if (const std::string config_path = Path::Combine(game_directory, LOCAL_CONFIG_FILENAME);
      ...
      !FileSystem::WriteStringToFile(config_path.c_str(), GetDefaultConfigurationString(), &error))
  {
...
std::string TextureReplacements::GetVRAMWriteDumpFilename(const VRAMReplacementName& name)
{
  std::string ret;
  if (!EnsureGameDirectoryExists())
    return ret;
...
std::string TextureReplacements::ExportConfiguration(const Settings::TextureReplacementSettings::Configuration& config,
                                                     bool comment)
{
...
#Aliases:
  ...
  return fmt::format(LOCAL_CONFIG_TEMPLATE, comment_str, config.dump_texture_pages, // DumpTexturePages
                     comment_str, config.dump_full_texture_pages,                   // DumpFullTexturePages
                     comment_str, config.dump_c16_textures,                         // DumpC16Textures
                     comment_str, config.reduce_palette_range,                      // ReducePaletteRange
                     comment_str, config.convert_copies_to_writes,                  // ConvertCopiesToWrites
                     comment_str, config.max_vram_write_splits,                     // MaxVRAMWriteSplits
                     comment_str, config.texture_dump_width_threshold,              // DumpTextureWidthThreshold
                     comment_str, config.texture_dump_height_threshold,             // DumpTextureHeightThreshold
                     comment_str, config.vram_write_dump_width_threshold,           // DumpVRAMWriteWidthThreshold
                     comment_str, config.vram_write_dump_height_threshold,          // DumpVRAMWriteHeightThreshold
                     comment_str, config.replacement_scale_linear_filter);          // ReplacementScaleLinearFilter
}

std::string TextureReplacements::GetDefaultConfigurationString()
{
  Settings::TextureReplacementSettings::Configuration config;
  return ExportConfiguration(config, true);
}
