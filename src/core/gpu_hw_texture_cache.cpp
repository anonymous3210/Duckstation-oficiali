...
void GPUTextureCache::DumpTexturesFromVRAMWrite(VRAMWrite* entry)
{
  if (g_settings.texture_replacements.dump_textures && !TextureReplacements::GetConfig().dump_texture_pages)
  {
    for (const VRAMWrite::PaletteRecord& prec : entry->palette_records)
    {
      ...
      TextureReplacements::DumpTexture(TextureReplacements::ReplacementType::TextureFromVRAMWrite, offset_x, offset_y,
                                       entry->write_rect.width(), entry->write_rect.height(), prec.key.mode,
                                       entry->hash, pal_hash, pal_min, pal_max, prec.palette, prec.rect, prec.flags);
    }
  }
}
...
{
  // C16 filter
  if (!TextureReplacements::GetConfig().dump_c16_textures && src->key.mode >= GPUTextureMode::Direct16Bit)
    return;

  const bool dump_full_page = TextureReplacements::GetConfig().dump_full_texture_pages;
  ...
  if (src->key.HasPalette() && TextureReplacements::GetConfig().reduce_palette_range)
  {
  ...
  TextureReplacements::DumpTexture(TextureReplacements::ReplacementType::TextureFromPage,
                                   ApplyTextureModeShift(src->key.mode, dump_offset_in_page.x), dump_offset_in_page.y,
                                   unwrapped_texture_rect.width(), unwrapped_texture_rect.height(), src->key.mode,
                                               ...
                                               HashCacheEntry* entry)
{
  ...
  {
    TextureReplacements::GetTexturePageTextureReplacements(subimages, key.page, tex_hash, pal_hash, key.mode,
                                                           key.palette);
  }
